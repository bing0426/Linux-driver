...
int my_platform_driver_probe(struct platform_device * my_platform_dev){
    ...
    //gpio相关操作
    my_led_device.gpio_num = of_get_named_gpio(my_platform_dev->dev.of_node, "myled_gpios", 0);
    ...
    //key相关操作
    key_nod = of_find_node_by_path("/mykeys");
    if(key_nod == NULL){
        printk("of_find_node_by_path fail \n");
    }
    my_key.key1_irq = of_irq_get(key_nod, 0);
    if(my_key.key1_irq < 0){
        printk("of_irq_get fial\n");
        return my_key.key1_irq;
    }
    printk("key1 irq:%d %d\n",my_key.key1_irq, __LINE__);
    ...
    my_key.key2_irq = of_irq_get(key_nod, 1);
